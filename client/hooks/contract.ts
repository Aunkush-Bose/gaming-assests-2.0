...
export const CONTRACT_ADDRESS =
  "CBX2YDCLBBZQYOMYHWRI6IYDI6V4PYVNHD7NNJRKRTCFGUKMMM34NI7B";
...
// ============================================================
// Gaming Assets — Contract Methods (manual ScVal wrappers)
// ============================================================

/**
 * Convert string to Symbol for contract calls
 */
 ...
 * Calls: mint_asset(player: Address, asset_id: Symbol, metadata: Symbol)
 */
export async function mintAsset(
  ...
  return callContract(
    "mint_asset",
    [new Address(caller).toScVal(), toScValSymbol(assetId), toScValSymbol(metadata)],
    caller,
    true
  );
}
 ...
 * Calls: transfer_asset(from: Address, to: Address, asset_id: Symbol)
 */
export async function transferAsset(
  ...
  return callContract(
    "transfer_asset",
    [new Address(caller).toScVal(), new Address(to).toScVal(), toScValSymbol(assetId)],
    caller,
    true
  );
}
...
/**
 * Get asset metadata (read-only).
 * Calls: get_asset(player: Address, asset_id: Symbol) -> Symbol
 */
export async function getAsset(
  ...
  const result = await readContract(
    "get_asset",
    [new Address(player).toScVal(), toScValSymbol(assetId)],
    player
  );
  return result;
}
